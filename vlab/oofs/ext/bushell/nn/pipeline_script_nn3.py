"""
This script uses boundary_batch_plant_surrogate_model.pt to train an optimizer network
...
The optimizer network also has weight decay for L2 regularization to help prevent overfitting.
...
surrogate_model_path = "boundary_batch_plant_surrogate_model.pt"
optimizer_model_path = "optimizer_net.pt"

diversity_amount = 0.5  # increased to force more diversity
...
if __name__ == "__main__":
    ...
    # Compute total number of samples (num_runs) based on epochs and batch_size.
    ...
    for restart in range(num_restarts):
        ...
        # Create the optimizer with consistent settings (lr=1e-2, weight_decay=1e-5) and scheduler
        optimizer = torch.optim.Adam(optimizer_net.parameters(), lr=1e-2, weight_decay=1e-5)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1000, gamma=0.5)
        
        # Get number of epochs as before; use consistent training noise settings:
        param_noise_std = 0.02  # same as other scripts
        
        for idx in range(num_runs):
            optimizer.zero_grad()
            noise = torch.rand(batch_size, 1)
            params = optimizer_net(noise)
            # Add parameter noise and clamp
            ...
            if idx % max(1, num_runs // 100) == 0 or idx == num_runs - 1:
                ...
                # Denormalize the mean predicted surrogate cost for consistent reporting
