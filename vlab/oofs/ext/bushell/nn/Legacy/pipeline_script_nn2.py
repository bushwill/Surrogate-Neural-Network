...
from utils_nn_reporting import print_progress, get_csv_header, format_csv_row
from plant_comparison_nn import calculate_cost, read_real_plants
from surrogate_nn1 import PlantSurrogateNet

batch_size = 16

# Define the five surrogate model files to check
surrogate_models = [
    ...
    if os.path.exists(surrogate_model_path):
        ...
        if os.path.exists(csv_file):
            ...
            with open(csv_file, "a", newline="") as f:
                import csv
                writer = csv.writer(f)
                writer.writerow(get_csv_header())  # Use consistent CSV header
        ...
        for restart in range(num_restarts):
            ...
            for step in range(num_runs):
                ...
                pred_cost = torch.clamp(pred_cost, min=0)  # Ensure no negative cost predictions
                loss = pred_cost.mean()
